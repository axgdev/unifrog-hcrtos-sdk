...
static unsigned long st7789v2_read_panel_id(void);
...
static int st7789v2_detected_init = -1;
...
static const uint8_t st7789v2_init_009306[] = {
	RGB_CLK_NORMAL,
	1, SLPOUT, 0,
	1, 0xfe, 0,
	1, 0xef, 0,
	...
	3, 0xa8, 0x13, 0x13, 0,
	3, 0xa9, 0x25, 0x25, 0,
	5, CASET, 0x00, 0x00, 0x00, 0xef, 0,
	5, RASET, 0x00, 0x00, 0x01, 0x3f, 0,
	1, RAMWR, 0,
...
static struct {
	...
	INIT_START_END(st7789v2_init_009306)
};
...
static void st7789v2_apply_init_sequence(int init_index)
{
	...
	if (*pinit++ == RGB_CLK_SKEWED) {
		...
		*(volatile unsigned *)0xb8800078 &= ~(1 << 15);
	}
...
unsigned long sf2000_lcd_panel_id(void)
{
	return st7789v2_detected_lcd_id;
}
...
static int st7789v2_read_register(unsigned short reg, unsigned short *data,
	unsigned count)
{
	unsigned i;
...
static int st7789v2_select_init_from_panel(void)
{
	...
	st7789v2_detected_lcd_id = lcd_id;
	...
	if (lcd_id == 0x858552) {
		...
		st7789v2_detected_aux_id =
			((unsigned long)(rb3[1] & 0xff) << 24) |
			((unsigned long)(rb3[2] & 0xff) << 16) |
			((unsigned long)(rf2[1] & 0xff) << 8) |
			(rf2[2] & 0xff);
	} else {
		st7789v2_read_register(0x00, r00, 2);
		st7789v2_detected_aux_id =
			((unsigned long)(r00[0] & 0xff) << 8) |
			(r00[1] & 0xff);
	}
	...
	case 0x858552:
		 ...
		 */
		if (rb3[1] == 0x00 && rb3[2] == 0x0f)
			init_index = INIT_X60_NEW;
		break;
	case 0x009329:
	...
	printf("st7789v2 panel probe r04=%02x:%02x:%02x:%02x lcd_id=0x%06lx aux=0x%08lx init=%d\n",
		r04[0] & 0xff, r04[1] & 0xff, r04[2] & 0xff, r04[3] & 0xff,
...
int sf2000_lcd_panel_brightness_probe(unsigned char brightness, unsigned char cabc)
{
    lcd_pinmux_rgb(0);
    lcd_configure_gpio_output();

    /*
     * ST7789-style panel brightness commands. These affect the LCD controller's
     * pixel drive/CABC path when supported by the panel, not the LED backlight
     * enable line.
     */
    st7789v2_write_command(0x53);
    st7789v2_write_data(0x2c);
    st7789v2_write_command(0x51);
    st7789v2_write_data(brightness);
    st7789v2_write_command(0x55);
    st7789v2_write_data(cabc);

    lcd_pinmux_rgb(1);
    printf("sf2000 lcd panel brightness probe brightness=%u cabc=%u\n",
        brightness, cabc);
    return 0;
}
...
static unsigned long st7789v2_read_panel_id(void)
{
	...
	st7789v2_write_command(0x04);
	lcd_configure_gpio_input();
	id[0] = st7789v2_read_data() & 0xff;
	id[1] = st7789v2_read_data() & 0xff;
	id[2] = st7789v2_read_data() & 0xff;
	id[3] = st7789v2_read_data() & 0xff;
	lcd_configure_gpio_output();

	printf("st7789v2 panel r04=%02x:%02x:%02x:%02x\n",
		id[0], id[1], id[2], id[3]);
	return ((unsigned long)id[1] << 16) | ((unsigned long)id[2] << 8) | id[3];
}
