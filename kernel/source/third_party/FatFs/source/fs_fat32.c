...
struct fat_mountpt_s {
	...
	BYTE pdrv;                       /* FatFs physical drive slot */
	char drive[5];                   /* FatFs logical drive path, e.g. "0:" */
	FATFS fatfs;
};
...
static char *fat_make_path(struct fat_mountpt_s *fs, const char *relpath)
{
	char *path;
	const char *sep;
	size_t drive_len;
	...
	drive_len = strlen(fs->drive);
	rel_len = strlen(relpath);
	if (rel_len == 0) {
		sep = "/";
	} else {
		sep = relpath[0] == '/' ? "" : "/";
	}
	path_len = drive_len + strlen(sep) + rel_len + 1;

	path = (char *)kmm_malloc(path_len);
	if (!path)
		return NULL;
	snprintf(path, path_len, "%s%s%s", fs->drive, sep, relpath);
	return path;
}
                    ...
                    int oflags, mode_t mode)
{
	...
	path = fat_make_path(fs, relpath);
	if (!path) {
		kmm_free(ff);
		return -ENOMEM;
	}

	res = f_open(&ff->fil, path, fmode);
	kmm_free(path);
	if (res != FR_OK) {
		kmm_free(ff);
		return fatfs_errno(res);
	}
	if ((uint32_t)oflags & O_APPEND) {
		res = f_lseek(&ff->fil, f_size(&ff->fil));
		if (res != FR_OK) {
			f_close(&ff->fil);
			kmm_free(ff);
			return fatfs_errno(res);
		}
		filep->f_pos = f_tell(&ff->fil);
	}
                         ...
                         size_t buflen)
{
	...
	if ((uint32_t)filep->f_oflags & O_APPEND) {
		res = f_lseek(&ff->fil, f_size(&ff->fil));
		if (res != FR_OK) {
			return fatfs_errno(res);
		}
...
static int fat_mount(struct fat_mountpt_s *fs, bool writeable)
{
	...
	if (fatfs_register_blkdrv(&fs->blkdrv, &fs->pdrv) != 0) {
		log_e("ERROR: No free FatFs drive slots\n");
		ret = -EBUSY;
		goto errout;
	}
	snprintf(fs->drive, sizeof(fs->drive), "%u:", (unsigned)fs->pdrv);

	/* We did it! */
	res = f_mount(&fs->fatfs, fs->drive, 1);
	if (res != FR_OK) {
		f_unmount(fs->drive);
		fatfs_unregister_blkdrv(fs->pdrv, &fs->blkdrv);
...
static int fat_statfs(FAR struct inode *mountpt, FAR struct statfs *buf)
{
	...
	res = f_getfree(fs->drive, &freeClust, &fatfs);
	if (res != FR_OK) {
		return fatfs_errno(res);
	}
	if (!fatfs)
	...
	buf->f_bsize = fatfs->ssize * fatfs->csize;
#else
	buf->f_bsize = FF_MIN_SS * fatfs->csize;
#endif
#if FF_USE_LFN == 0
               ...
               FAR const char *newrelpath)
{
	...
	oldpath = fat_make_path(fs, oldrelpath);
	newpath = fat_make_path(fs, newrelpath);
	if (!oldpath || !newpath) {
		kmm_free(oldpath);
		kmm_free(newpath);
		return -ENOMEM;
                    ...
                    FAR struct stat *buf)
{
	...
	fat_date_t cdate = { .as_int = finfo.crdate };
	fat_time_t ctime = { .as_int = finfo.crtime };
...
int mkfatfs(const char *pathname, struct fat_format_s *fmt)
{
	...
	if (fatfs_register_blkdrv(&blkdrv, &pdrv) != 0) {
		log_e("ERROR: No free FatFs drive slots\n");
		ret = -EBUSY;
		goto errout_with_driver;
	}
	...
	res = f_mkfs(drive, 0, work, sizeof work);
	if (res != FR_OK) {
	...
	fatfs_unregister_blkdrv(pdrv, &blkdrv);
	close(blkdrv.fs_fd);
	return OK;
